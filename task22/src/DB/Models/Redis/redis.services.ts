...
class RedisService {
  ...
  async set(
    ...
    const options =
      expire && expire > 0 ? { EX: Math.floor(expire) } : undefined;
  ...
  async addFcmTokenToset(userId:Types.ObjectId|string,token:string){
    return  await client.sAdd(this.getFcmKey(userId),token)
  }
  async getMemberFcmToken(userId:Types.ObjectId|string){
    return await client.sMembers(this.getFcmKey(userId))
  }
   getSocketTOKey(userId:Types.ObjectId|string){
    return `SocketID::${userId}`;
  }
  async addSocketIoIDToSet(userId:Types.ObjectId|string,SocketId:string){
    return  await client.sAdd(this.getSocketTOKey(userId),SocketId)
  }
    async removeSocketIoIDToSet(userId:Types.ObjectId|string,SocketId:string){
    return  await client.sRem(this.getSocketTOKey(userId),SocketId)
  }
